        ...
        for i in set(df1.index).union(set(df2.index)):
            for j in set(df1.columns).union(set(df2.columns)):
                ...
                if i in df2.index and j in df2.columns and not df2.loc[i,j] == 0:
                    f.write("\t".join([str(i), str(j), str(df2.loc[i,j]), "phypat+PGL"]) + "\n")
...
def comb_preds(phypat_dir, phypat_GGL_dir, out_dir, k):
    ...
    m2_maj = ps.read_csv("%s/predictions_majority-vote.txt"%phypat_GGL_dir, index_col = 0, sep = "\t")
    m2 = ps.read_csv("%s/predictions_single-votes.txt"%phypat_GGL_dir, index_col = 0, sep = "\t")
